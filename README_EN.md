...
│
├── hybridRetriever/          # 4. Hybrid retrieval (BM25 / keyword / RRF / Milvus sparse+dense)
│   ├── __init__.py           #   Public API re-exports
│   ├── bm25_retriever.py     #   BM25 (rank_bm25, Okapi/Plus/L)
│   ├── keyword_retriever.py  #   Keyword (TFIDF + KeywordTable)
│   ├── vector_retriever.py   #   Dense vector + score normalization
│   ├── hybrid_retriever.py   #   Multi-route fusion (HybridRetriever + factory)
│   ├── milvus_hybrid.py      #   Milvus native enable_sparse
│   ├── utils.py              #   rrf_fuse / weighted_fuse / tokenize etc.
│   ├── demo.py               #   4 end-to-end demos
│   ├── README.md             #   Chinese docs
│   └── README_EN.md          #   English docs
│
└── ragasEvaluator/           # 5. RAGAS evaluation (end-to-end RAG quality assessment)
    ├── __init__.py           #   Public API re-exports
    ├── RAGASevaluatorUtil.py #   RAGASEvaluator (wraps ragas.evaluate)
    ├── testset_generator.py  #   TestsetGenerator (LLM-generated QA pairs)
    ├── rag_runner.py         #   RAGRunner (wraps RAG queries, outputs answer+contexts)
    ├── pipeline.py           #   RAGASEvalPipeline + run_ragas_eval one-stop factory
    ├── run_eval_demo.py      #   CLI demo entry point
...
### 4.4 `hybridRetriever` — Hybrid Retrieval
| Function / Class | Description |
|---|---|
| `BM25Retriever` | BM25 retrieval (Okapi / Plus / L, based on rank_bm25) |
| `TFIDFKeywordRetriever` | TF-IDF keyword retrieval (inverted index, pure Python) |
| `LlamaIndexKeywordRetriever` | LlamaIndex KeywordTableIndex (simple / rake) |
| `VectorRetriever` | Dense vector retrieval (VectorStoreIndex wrapper + score normalization) |
| `HybridRetriever` | Multi-route fusion retrieval (RRF / weighted / concatenation) |
| `build_hybrid_retriever()` | One-stop factory for building a hybrid retriever |
| `build_milvus_hybrid_store()` | Milvus native enable_sparse dual-vector store |
| `MilvusHybridRetriever` | Milvus native hybrid retrieval |
| `rrf_fuse()` / `weighted_fuse()` | RRF fusion / weighted fusion |
| `score_normalize()` / `tokenize()` | Score normalization / Chinese tokenization |

### 4.5 `ragasEvaluator` — RAGAS Evaluation
| Function / Class | Description |
|---|---|
| `RAGASEvaluator` | RAGAS evaluator (wraps `ragas.evaluate`, supports faithfulness / answer_relevancy / context_precision / context_recall) |
| `TestsetGenerator` | Auto-generate (question, ground_truth) QA pairs from chunks using LLM |
| `RAGRunner` | Wraps RAG engine calls, outputs (answer, contexts) |
| `RAGASEvalPipeline` | End-to-end evaluation pipeline (object-oriented, step-by-step control) |
| `run_ragas_eval()` | **One-stop factory function**, run the entire pipeline in one line |

Evaluation pipeline:
```
Raw docs → Split → Feed to RAG index (VectorStoreIndex)
                  → Feed to LLM to generate Q&A pairs (question, ground_truth)
                          → RAGRunner executes queries (answer, contexts)
                          → Align by question and merge
                          → RAGASEvaluator computes metrics
```
...
**Q14. ragas reports `Unknown model` when using OpenAI-compatible APIs (DeepSeek / dmxapi etc.)?**
ragas creates an OpenAI native client by default and validates model names against a whitelist. Third-party model names are rejected. This project bypasses the restriction via `LangchainLLMWrapper` + `langchain_openai.ChatOpenAI`. `RAGASEvaluator` handles this automatically — no manual configuration needed.

**Q15. LlamaIndex reports `Unknown model` or 404 when using OpenAI-compatible APIs?**
LlamaIndex's `OpenAI` LLM validates model names against `ALL_AVAILABLE_MODELS` and `CHAT_MODELS`. Models not in the list fall back to the text completions endpoint (which most third-party APIs don't support). This project's `get_llm()` automatically patches the registry — no manual handling needed.
...
```bash
python -m ragasEvaluator.run_eval_demo --file xxx.pdf --splitter sentence --n-qa 1
```

**Option C: Step-by-step control (object-oriented)**

```python
from ragasEvaluator import RAGASEvalPipeline, RAGASEvaluator

pipeline = RAGASEvalPipeline(
    index=my_index,
    n_questions_per_chunk=2,
    similarity_top_k=3,
    use_chat_engine=False,  # Recommended for evaluation (no memory)
)
...
| `answer_relevancy` | Answer relevancy: is the answer relevant to the question? |
| `context_precision` | Context precision: proportion of useful information in retrieved context |
| `context_recall` | Context recall: coverage of ground-truth information in the context |

### 9.3 OpenAI-Compatible API Notes

`RAGASEvaluator` internally constructs ragas-compatible LLM via `langchain_openai.ChatOpenAI` + `LangchainLLMWrapper`,
and ragas-compatible Embeddings via `langchain_openai.OpenAIEmbeddings` + `LangchainEmbeddingsWrapper`.
It automatically reads `LLM` and `EMBED` config from `config.py` — no manual passing needed.
Third-party model names (e.g. `deepseek-v4-flash`) will not trigger ragas model name validation errors.
