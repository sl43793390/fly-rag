...
from dataLoader import auto_load, get_doc_type, list_supported_files, load_anydoc, load_web
...
def ingest_file(file_path: Union[str, Path]) -> VectorStoreIndex:
    ...
    # 根据后缀选切分器(映射统一维护在 dataLoader.loaders.EXT_DOC_TYPE_MAP)
...
def ingest_directory(
    ...
    fail_count = 0
    total_nodes = 0
    # 切分器映射统一维护在 dataLoader.loaders.EXT_DOC_TYPE_MAP
    ...
    for i, file_path in enumerate(files, 1):
        ...
        try:
            ...
            # 3.2 切分(按文件后缀决定切分器类型)
            file_doc_type = get_doc_type(file_path)
...
# anydoc 测试
# ============================================================
def test_anydoc(file_path: Union[str, Path]) -> None:
    """
    测试 anydoc 加载器,解析文档并输出 markdown 内容。

    Args:
        file_path: 任意受支持格式的文件路径。
    """
    print(f"\n=== 测试 anydoc 加载: {file_path} ===")
    try:
        docs = load_anydoc(file_path)
        if docs:
            print(f"成功解析,得到 {len(docs)} 个 Document")
            print(f"内容预览(前 500 字符):\n{docs[0].text[:500]}")
        else:
            print("解析结果为空")
    except ImportError as e:
        print(f"anydoc 未安装: {e}")
...
if __name__ == "__main__":
    ...
    # 方式 D:使用 anydoc 解析多种办公文档格式(自动转换为 markdown)
    #   支持格式:.ppt/.pptx/.xls/.xlsx/.doc/.docx/.pdf/.epub/.csv/.rtf 等
    #   需要先安装 anydoc: pip install anydoc
    # test_anydoc("项目周报-样板.ppt")
    # test_anydoc("测试文档.xlsx")
    # test_anydoc("演示文稿.pptx")
